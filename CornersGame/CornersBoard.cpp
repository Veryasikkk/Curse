...
void CornersBoard::UpdateAvailableMoves_(CellType playerType, CellType opponentType)
{
	...
	for (auto i = _availableMoves.begin(); i != _availableMoves.end(); ++i) {
		if (i->second.size() == 0) {
			keysToDelete.push_back(i->first);
		}
	}
	for (auto i = keysToDelete.begin(); i != keysToDelete.end(); ++i) {
		_availableMoves.erase(*i);
	}
}
...
bool CornersBoard::IsGameFinished_()
{
	...
	if (whiteCellInCity == 9) {
		_bGameFinished = true;
		this->_winner = 2;
		return true;
	}
	if (blackCellInCity == 9) {
		_bGameFinished = true;
		this->_winner = 1;
		return true;
	}
...
bool CornersBoard::SetCellNoRule(int start_row, int start_col, int end_row, int end_col, CellType type)
{
	cells[start_row][start_col] = CellType_Empty;
	cells[end_row][end_col] = type;
	_currentTurn--;
...
std::unordered_map<int, std::unordered_set<int>> CornersBoard::GetPossibleMoves()
{
...
unsigned int CornersBoard::GetBoardsize()
{
